...
static int ov5642_set_idle_mode(void) {
	...
	pr_info("Checking if AF status is idle....");
	ReadVal = -1;
...
//			ov5642_write_reg(0x5183 ,0x0 );
		...
		case V4L2_WHITE_BALANCE_INCANDESCENT:
			pr_info("Setting AWB to INCANDESCENT");

			//based on DellStreak 5
			ov5642_write_reg(REG_AWB_MANUAL, 		0x01);
			ov5642_write_reg(REG_AWB_R_GAIN_HIGH, 	0x2);
			ov5642_write_reg(REG_AWB_R_GAIN_LOW, 	0xc4);
			ov5642_write_reg(REG_AWB_G_GAIN_HIGH, 	0x3);
			ov5642_write_reg(REG_AWB_G_GAIN_LOW, 	0x0);
			ov5642_write_reg(REG_AWB_B_GAIN_HIGH, 	0x6);
			ov5642_write_reg(REG_AWB_B_GAIN_LOW, 	0xc8);
			retval=0;
			break;
		case V4L2_WHITE_BALANCE_DAYLIGHT:
			pr_info("Setting AWB to DAYLIGHT");
			ov5642_write_reg(REG_AWB_MANUAL, 		0x01 );
			ov5642_write_reg(REG_AWB_R_GAIN_HIGH, 	0x5 );
			ov5642_write_reg(REG_AWB_R_GAIN_LOW, 	0x80 );
			ov5642_write_reg(REG_AWB_G_GAIN_HIGH, 	0x4 );
			ov5642_write_reg(REG_AWB_G_GAIN_LOW, 	0x0 );
			ov5642_write_reg(REG_AWB_B_GAIN_HIGH, 	0x5 );
			ov5642_write_reg(REG_AWB_B_GAIN_LOW, 	0x38 );
			retval=0;
			break;
		case V4L2_WHITE_BALANCE_FLUORESCENT:
			pr_info("Setting AWB to FLUORESCENT");
			ov5642_write_reg(REG_AWB_MANUAL, 		0x01 );
			ov5642_write_reg(REG_AWB_R_GAIN_HIGH, 	0x3 );
			ov5642_write_reg(REG_AWB_R_GAIN_LOW, 	0xb2 );
			ov5642_write_reg(REG_AWB_G_GAIN_HIGH, 	0x3 );
			ov5642_write_reg(REG_AWB_G_GAIN_LOW, 	0x0 );
			ov5642_write_reg(REG_AWB_B_GAIN_HIGH, 	0x4 );
...
static int ov5642_auto_focus_start(void) {
	...
	if (retval > -1) {
		...
		RegAddr = REG_CMD_MAIN;
		pr_info("Sending cmd 0x%x to camera...", RegVal);
		...
		for (lc = 0; (lc < 500) && (RegVal != S_FOCUSED); ++lc) {
			RegVal = 0x00;
			retval = ov5642_read_reg(RegAddr, &RegVal);
			if (retval < 0) {
				pr_err("%s, read reg 0x%x failed\n", __FUNCTION__, RegAddr);
...
static int ioctl_s_ctrl(struct v4l2_int_device *s, struct v4l2_control *vc)
{
	...
	case V4L2_CID_AUTO_N_PRESET_WHITE_BALANCE:
		pr_info(">>>> Received V4L2_CID_AUTO_N_PRESET_WHITE_BALANCE command in ov5642.c <<<<");
		pr_info("AWB Value passed: 0x%x", vc->value);
		retval = ov5642_set_awb(vc->value);
		break;
	case V4L2_CID_CONTRAST:
		break;
	case V4L2_CID_SATURATION:
...
static int ioctl_send_command(struct v4l2_int_device *s, struct v4l2_send_command_control *vc) {
	...
	switch (vc->id) {
		...
		case 105: //step to specified position
			pr_info("\nStepping to specified position");
...
static int ioctl_dev_init(struct v4l2_int_device *s)
{
	...
	msleep(300);
	pr_info("Writing Truly firmware");
	pModeSetting = ov5642_af_firmware;
	iModeSettingArySize = ARRAY_SIZE(ov5642_af_firmware);
